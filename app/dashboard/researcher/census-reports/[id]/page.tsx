...
export default function ResearcherCensusReportDetailPage() {
      ...
      <main className="mx-auto max-w-4xl px-4 py-4 pb-24">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            {!!String(report.electoral_district || '').trim() && (
              <p className="text-sm sm:col-span-2">
                <span className="font-semibold">Electoral district (NEC):</span>{' '}
                <span className="text-gray-800" title={electoralLabelFor(report.county, report.electoral_district)}>
                  {report.electoral_district}
                </span>
                <span className="mt-1 block text-xs text-gray-500">{electoralLabelFor(report.county, report.electoral_district)}</span>
              </p>
            )}
