...
import { DISTRICTS_BY_COUNTY, LIBERIA_COUNTIES } from '@/lib/locations/liberia';
...
export default function EditResearcherCensusReportPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const { isAuthorized, isLoading } = useRoleRedirect(['researcher', 'admin', 'management']);
  const { gate: accessGate, navProfile } = useCensusReportsAccess(isAuthorized);
  ...
  useEffect(() => {
    if (!isAuthorized || accessGate !== 'ok' || !params?.id) return;
    ...
    fetchReport();
  }, [isAuthorized, accessGate, params?.id]);

  const selectedCounty = LIBERIA_COUNTIES.find((county) => normalize(county) === normalize(form.county)) || '';
  const districtOptions = useMemo(
    () => (selectedCounty ? DISTRICTS_BY_COUNTY[selectedCounty] || [] : []),
    [selectedCounty]
  );
  ...
  const resolvePlaceFromCoordinates = async (lat: number, lng: number, autoApply = false) => {
    ...
    try {
      ...
      setForm((prev) => {
        ...
        if (matchedCounty && !prev.county) {
          next.county = matchedCounty;
          next.district = '';
        }
        if (cityName && !prev.community) {
  ...
  useEffect(() => {
    ...
    if (resolvedPlaceName) return;
    resolvePlaceFromCoordinates(form.gps_lat, form.gps_lng, true);
  }, [loadingReport, form.gps_lat, form.gps_lng, resolvedPlaceName]);
  ...
  const validateForm = (): string | null => {
    ...
    if (form.district.trim() && !districtOptions.some((name) => normalize(name) === normalize(form.district))) {
      return 'District does not belong to selected county';
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => router.push('/dashboard/researcher/census-reports'), 450);
    } catch (error) {
  ...
  if (isLoading || !isAuthorized || accessGate !== 'ok' || loadingReport) {
    return (
      ...
      <header className="sticky top-0 z-30 border-b border-emerald-100 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => router.push('/dashboard/researcher/census-reports')}
        ...
        <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="grid gap-3">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <label className="grid gap-1">
                ...
                <select
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, county: e.target.value, district: '' }))}
                  className="rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none"
                  required
                >
              ...
              <label className="grid gap-1">
                <span className="text-xs font-medium text-gray-600">District (optional)</span>
                <select
                  value={form.district}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1">
                <span className="text-xs font-medium text-gray-600">GPS Latitude (fallback)</span>
              ...
              <label className="grid gap-1">
                <span className="text-xs font-medium text-gray-600">GPS Longitude (fallback)</span>
