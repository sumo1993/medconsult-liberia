...
export default function ManagementDirectMessagesPage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('dm_pins_management');
      if (raw) {
  ...
  const togglePin = (userId: number) => {
    setPinnedUserIds((prev) => {
      ...
      localStorage.setItem('dm_pins_management', JSON.stringify(next));
      return next;
    });
  };
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-600 sm:text-sm">Chat with anyone in the system</p>
              </div>
            </div>
