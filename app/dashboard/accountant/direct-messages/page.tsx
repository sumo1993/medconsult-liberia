...
export default function AccountantDirectMessagesPage() {
  const router = useRouter();
  const { markCategorySeen } = useNotifications('accountant');
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('dm_pins_accountant');
      if (raw) {
  ...
  const togglePin = (userId: number) => {
    setPinnedUserIds((prev) => {
      ...
      localStorage.setItem('dm_pins_accountant', JSON.stringify(next));
      return next;
    });
  };
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-600 sm:text-sm">Chat with Admin &amp; CEO</p>
              </div>
            </div>
            <div className="flex shrink-0 flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="whitespace-nowrap rounded-lg bg-yellow-600 px-3 py-2 text-sm text-white hover:bg-yellow-700 sm:px-4"
              >
                New Message
              </button>
            </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg bg-white shadow md:h-[calc(100vh-200px)] md:min-h-[400px] md:flex-row">
                ...
                <div className="overflow-y-auto" style={{ height: 'calc(100% - 57px)' }}>
                  {conversations.length === 0 ? (
                    <div className="p-8 text-center text-gray-500">
                      ...
                      <button
                        onClick={() => setShowNewChat(true)}
                        className="mt-3 text-yellow-600 hover:underline text-sm"
                      >
                        Start a new chat
                      </button>
                    </div>
                    ...
                    sortedConversations.map((conv) => (
                      <div
                        ...
                        className={`p-4 border-b cursor-pointer hover:bg-gray-50 ${
                          selectedUser?.id === conv.user_id ? 'bg-yellow-50' : ''
                        }`}
                      >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`text-xs px-2 py-0.5 rounded-full ${getRoleBadgeColor(conv.role)}`}>
                                {conv.role === 'management' ? 'CEO' : conv.role}
                              </span>
                              {conv.unread_count > 0 && (
              ...
              >
                {selectedUser ? (
                  <>
                    {/* Chat Header */}
                    <div className="p-4 border-b bg-gradient-to-r from-yellow-600 to-yellow-700 text-white">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">
                            {selectedUser.role === 'management' ? 'CEO' : selectedUser.role}
                          </span>
                        </div>
                      </div>
                    </div>
                            ...
                            <div key={msg.id}>
                              ...
                              {showUnreadDivider && (
                                <div className="my-2 flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] uppercase font-semibold text-yellow-600">New Messages</span>
                                  <div className="h-px flex-1 bg-yellow-200" />
                                </div>
                              )}
                              <div className={`flex mb-4 ${isOwn ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[min(92vw,85%)] sm:max-w-[70%] ${isOwn ? 'order-2' : ''}`}>
                                  ...
                                  <div className={`flex items-center gap-1 mt-1 text-xs text-gray-500 ${isOwn ? 'justify-end' : ''}`}>
                                    ...
                                    {isOwn && (
                                      msg.is_read
                                        ? <CheckCheck size={14} className="text-yellow-500" />
                                        : <Check size={14} />
                                    )}
                                  </div>
                    ...
                    <div className="shrink-0 border-t bg-white px-4 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
                      ...
                      {replyTo && (
                        <div className="mb-2 rounded-lg border border-yellow-200 bg-yellow-50 p-2 text-xs text-yellow-900 flex items-center justify-between">
                      ...
                      <div className="flex items-end gap-2">
                        ...
                        <button
                          ...
                          className="p-3 bg-yellow-600 text-white rounded-full hover:bg-yellow-700 disabled:opacity-50"
                        >
                  ...
                  <div className="flex-1 flex items-center justify-center text-gray-500">
                    <div className="text-center">
                      ...
                      <p className="text-sm">or start a new chat with Admin / CEO</p>
                    </div>
                  </div>
                )}
              </div>
                ...
                {Object.entries(usersByRole).map(([role, roleUsers]) => (
                  <div key={role} className="mb-4">
                    <h4 className={`text-xs font-semibold uppercase px-2 py-1 rounded ${getRoleBadgeColor(role)} mb-2`}>
                      {role === 'management' ? 'CEO / Management' : role} ({roleUsers.length})
                    </h4>
                    {roleUsers.map((user) => (
                      <div
                        key={user.id}
